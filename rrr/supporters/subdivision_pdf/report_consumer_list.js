...
function callreport_consumer_list(month_data, month_year, supply_cat, fonts, AccessToken, ipAddr, invoked_date, filepath, subdiv_name) {
	...
	return new Promise(function (resolve) {
		...
		try {
			...
			table_format.push({ text: 'Consumer List Report', style: 'header' })
			table_format.push({
				style: 'tableExample',
				layout: 'noBorders',
				table: {
			...
			pdfDoc.pipe(fs.createWriteStream(filepath + invoked_date + "/" + subdiv_name + "/" + supply_cat + "_Consumer_List_Report.pdf"));
			pdfDoc.end();

			global.status.cnt = global.status.cnt - 1
			console.log("Consumer List Over : " + global.status.cnt)
...
module.exports = { callreport_consumer_list: callreport_consumer_list };