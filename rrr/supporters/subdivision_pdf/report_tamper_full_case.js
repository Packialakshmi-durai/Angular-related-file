...
function callreport_tamper_full_case(month_data, month_year, supply_cat, fonts, AccessToken, ipAddr, invoked_date, filepath, subdiv_name) {
	...
	return new Promise(function (resolve) {
		...
		try {
			...
			if (dm.length != 0) {
				...
				table_format.push({
					...
					table: {
						...
						body: [
							...
							[{ text: 'Division Name', style: 'contentHeader' }, dm[0]['P2'], { text: 'Division Name', style: 'contentHeader' }, dm[0]['P3']]
						]
					}
				})
				...
				pdfDoc.pipe(fs.createWriteStream(filepath + invoked_date + "/" + subdiv_name + "/" + supply_cat + "_Tamper_Full_Cases_Report.pdf"));
				pdfDoc.end();
