...
function callreport_tamper_not_found(month_data, month_year, supply_cat, fonts, AccessToken, ipAddr, invoked_date, filepath, subdiv_name) {
	...
	return new Promise(function (resolve) {
		...
		try {
			...
			for (var d = 0; d < dm_all.length; d++) {
				if (dm_all[d]['P67'] == "No data" && dm_all[d]['P68'] == null && dm_all[d]['P69'] == null && dm_all[d]['P70'] == null && dm_all[d]['P71'] == null && dm_all[d]['P72'] == null) {
					dm.push(dm_all[d])
				}
			}
			...
			if (dm.length != 0) {
				...
				table_format.push({ text: 'Tamper Not Available Report', style: 'header' })
				table_format.push({
					style: 'tableExample',
					layout: 'noBorders',
					table: {
				...
				pdfDoc.pipe(fs.createWriteStream(filepath + invoked_date + "/" + subdiv_name + "/" + supply_cat + "_Tamper_Not_Found_Report.pdf"));
				pdfDoc.end();

				global.status.cnt = global.status.cnt - 1
				console.log("D5 Not Found Over : " + global.status.cnt)
...
module.exports = { callreport_tamper_not_found: callreport_tamper_not_found };