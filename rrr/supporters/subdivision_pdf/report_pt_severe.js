...
function callreport_pt_severe(month_data, month_year, supply_cat, fonts, AccessToken, ipAddr, invoked_date, filepath, subdiv_name) {
	...
	return new Promise(function (resolve) {
		...
		try {
			...
			for (var d = 0; d < dm_all.length; d++) {
				if (dm_all[d]['P20'] != null && dm_all[d]['P22'] != null && dm_all[d]['P24']) {
					if (parseFloat(dm_all[d]['P20']) < 0.7 || parseFloat(dm_all[d]['P22']) < 0.7 || parseFloat(dm_all[d]['P24']) < 0.7)
						dm.push(dm_all[d])
				}
			}
			...
			if (dm.length != 0) {
				...
				table_format.push({ text: 'PT Unbalance Severe Report', style: 'header' })
				table_format.push({
					style: 'tableExample',
					layout: 'noBorders',
					table: {
				...
				{ text: 'V-R volt', style: 'tableHeader' }, { text: 'V-Y volt', style: 'tableHeader' },
				{ text: 'V-B volt', style: 'tableHeader' }, { text: 'Meter RTC', style: 'tableHeader' }])
				...
				for (var i = 0; i < dm.length; i++) {
					...
					{ text: vr, style: 'tableRows' }, { text: vy, style: 'tableRows' },
					{ text: vb, style: 'tableRows' }, { text: dm[i]['P18'], style: 'tableRows' }])
				}
				...
				pdfDoc.pipe(fs.createWriteStream(filepath + invoked_date + "/" + subdiv_name + "/" + supply_cat + "_PT_Unbalance_Severe_Report.pdf"));
				pdfDoc.end();

				global.status.cnt = global.status.cnt - 1
				console.log("PT Severe Over : " + global.status.cnt)
...
module.exports = { callreport_pt_severe: callreport_pt_severe };