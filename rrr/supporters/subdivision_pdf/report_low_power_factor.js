...
function callreport_low_power_factor(month_data, month_year, supply_cat, fonts, AccessToken, ipAddr, invoked_date, filepath, subdiv_name) {
	...
	return new Promise(function (resolve) {
		...
		try {
			...
			for (var d = 0; d < dm_all.length; d++) {
				if (dm_all[d]['P63'] != null) {
					if (parseFloat(dm_all[d]['P63']) >= 50)
						dm.push(dm_all[d])
				}
			}
			...
			if (dm.length != 0) {
				...
				{ text: 'PF < 0.5 - Slot % >= 50', style: 'tableHeader' }, { text: 'PF 0.5 - 0.7 Slot %', style: 'tableHeader' },
				...
				pdfDoc.pipe(fs.createWriteStream(filepath + invoked_date + "/" + subdiv_name + "/" + supply_cat + "_Low_Power_Factor_Status_Report.pdf"));
				pdfDoc.end();

				global.status.cnt = global.status.cnt - 1
				console.log("Low Power Factor Over : " + global.status.cnt)
...
module.exports = { callreport_low_power_factor: callreport_low_power_factor };