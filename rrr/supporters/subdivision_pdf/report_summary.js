...
function callreport_summary(month_data, month_year, supply_cat, fonts, AccessToken, ipAddr, hierarchy_code, invoked_date, filepath, subdiv_name_main) {
                    ...
                    if (final_month_data.length != 0) {
                        ...
                        table_format.push({
                            ...
                            table: {
                                widths: ['*', '*', '*', '*'],
                                body: [
                                    [{ text: 'Report Month & Year', style: 'contentHeader' }, month_year, { text: 'Circle Name', style: 'contentHeader' }, final_month_data[0]['cirle']],
                                    [{ text: 'Division Name', style: 'contentHeader' }, final_month_data[0]['division'], { text: 'Sub Division Name', style: 'contentHeader' }, final_month_data[0]['subdivision']]
                                ]
                            }
                        })

                        table_rows.push([{ rowSpan: 2, text: 'SI No', style: 'tableHeader' },
                        { rowSpan: 2, text: 'Total No of Consumers', style: 'tableHeader' },
                        ...
                        for (var fnl = 0; fnl < final_month_data.length; fnl++) {
                            ...
                            table_rows.push([{ text: fnl + 1, style: 'tableRows' },
                            { text: final_month_data[fnl]['meter_count'], style: 'tableRows' },
                        ...
                        if (!fs.existsSync(subdiv_name_main)) {
                            fs.mkdirSync(subdiv_name_main);
                            process.chdir(filepath + invoked_date + "/" + subdiv_name_main + "/")
                        }
                        else {
                            process.chdir(filepath + invoked_date + "/" + subdiv_name_main + "/")
                        }
                        ...
                        pdfDoc.pipe(fs.createWriteStream(filepath + invoked_date + "/" + subdiv_name_main + "/" + supply_cat + "_Summary_Report.pdf"));
                        pdfDoc.end();
