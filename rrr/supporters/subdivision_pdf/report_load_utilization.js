...
function callreport_load_utilization(month_data, month_year, supply_cat, fonts, AccessToken, ipAddr, invoked_date, filepath, subdiv_name) {
	...
	return new Promise(function (resolve) {
		...
		try {
			...
			for (var d = 0; d < dm_all.length; d++) {
				if (dm_all[d]['P52'] != null && dm_all[d]['P53'] != null && dm_all[d]['P54'] != null && dm_all[d]['P55'] != null && dm_all[d]['P56'] != null && dm_all[d]['P57'] != null && dm_all[d]['P58'] != null && dm_all[d]['P59'] != null && dm_all[d]['P60'] != null && dm_all[d]['P61'] != null && dm_all[d]['P62'] != null) {
					dm.push(dm_all[d])
				}
			}
			...
			if (dm.length != 0) {
				...
				table_format.push({ text: 'Load Utilization Report', style: 'header' })
				table_format.push({
					style: 'tableExample',
					layout: 'noBorders',
					table: {
				...
				{ text: 'Demandunit', style: 'tableHeader' }, { text: 'AD', style: 'tableHeader' },
				{ text: 'MD / CD (%)', style: 'tableHeader' }, { text: 'AD / CD (%)', style: 'tableHeader' },
				{ text: 'AD / MD (%)', style: 'tableHeader' }, { text: 'CD < 20%-slot%', style: 'tableHeader' },
				...
				pdfDoc.pipe(fs.createWriteStream(filepath + invoked_date + "/" + subdiv_name + "/" + supply_cat + "_Load_Utilization_Report.pdf"));
				pdfDoc.end();

				global.status.cnt = global.status.cnt - 1
				console.log("Load Utilization Over : " + global.status.cnt)
...
module.exports = { callreport_load_utilization: callreport_load_utilization };