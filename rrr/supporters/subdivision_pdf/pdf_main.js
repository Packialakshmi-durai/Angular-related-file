...
var hierarchy_filter = function (month_data, month_year, AccessToken, supply_cat, hierarchy_code, fonts, filepath, from, to, invoked_date, report_type, pdf_name, report_count) {
	...
	for (var i = 0; i < month_data.length; i++) {
		subdiv_all.push(month_data[i]['P3'])
	}
	var subdiv_uniq = unique(subdiv_all)
	global.status.cnt = subdiv_uniq.length * report_count

	for (var s = 0; s < subdiv_uniq.length; s++) {
		setTimeout(async function (s) {

			var subdiv_name = subdiv_uniq[s]
			console.log(subdiv_name)

			var filterBy = { P3: [subdiv_name] },
				subdiv_result = month_data.filter(function (o) {
			...
			if (subdiv_result.length != 0) {
				if (report_type == "monthly")
					logic_call(subdiv_result, month_year, AccessToken, supply_cat, hierarchy_code, fonts, filepath, pdf_name, subdiv_name)
				if (report_type == "daily")
					report_daily.callreport_daily(subdiv_result, fonts, AccessToken, ipAddr, hierarchy_code, from, to, invoked_date, supply_cat, filepath, subdiv_name)
			}
		}, 1000 * s, s)
	}
}

var logic_call = function (month_data, month_year, AccessToken, supply_cat, hierarchy_code, fonts, filepath, pdf_name, subdiv_name) {

	try {

		if (pdf_name == "All") {

			report_instantaneous.callreport_instantaneous(month_data, month_year, supply_cat, fonts, AccessToken, ipAddr, invoked_date, filepath, subdiv_name)
			report_energy_comparison.callreport_energy_comparison(month_data, month_year, supply_cat, fonts, AccessToken, ipAddr, invoked_date, filepath, subdiv_name)
			report_md_exceeded.callreport_md_exceeded(month_data, month_year, supply_cat, fonts, AccessToken, ipAddr, invoked_date, filepath, subdiv_name)
			report_low_power_factor.callreport_low_power_factor(month_data, month_year, supply_cat, fonts, AccessToken, ipAddr, invoked_date, filepath, subdiv_name)
			report_low_load_utilization.callreport_low_load_utilization(month_data, month_year, supply_cat, fonts, AccessToken, ipAddr, invoked_date, filepath, subdiv_name)
			report_high_noload_blackout.callreport_high_noload_blackout(month_data, month_year, supply_cat, fonts, AccessToken, ipAddr, invoked_date, filepath, subdiv_name)
			report_tamper_full_case.callreport_tamper_full_case(month_data, month_year, supply_cat, fonts, AccessToken, ipAddr, invoked_date, filepath, subdiv_name)
			report_consumer_list.callreport_consumer_list(month_data, month_year, supply_cat, fonts, AccessToken, ipAddr, invoked_date, filepath, subdiv_name)
			report_consumption_based_analysis.callreport_consumption_based_analysis(month_data, month_year, supply_cat, fonts, AccessToken, ipAddr, invoked_date, filepath, subdiv_name)
			report_pt_severe.callreport_pt_severe(month_data, month_year, supply_cat, fonts, AccessToken, ipAddr, invoked_date, filepath, subdiv_name)
			report_load_survey_not_found.callreport_load_survey_not_found(month_data, month_year, supply_cat, fonts, AccessToken, ipAddr, invoked_date, filepath, subdiv_name)
			report_tamper_not_found.callreport_tamper_not_found(month_data, month_year, supply_cat, fonts, AccessToken, ipAddr, invoked_date, filepath, subdiv_name)
			report_power_factor.callreport_power_factor(month_data, month_year, supply_cat, fonts, AccessToken, ipAddr, invoked_date, filepath, subdiv_name)
			report_load_utilization.callreport_load_utilization(month_data, month_year, supply_cat, fonts, AccessToken, ipAddr, invoked_date, filepath, subdiv_name)
			report_seal.callreport_seal(month_data, month_year, supply_cat, fonts, AccessToken, ipAddr, invoked_date, filepath, subdiv_name)
			report_detective_mechanical.callreport_detective_mechanical(month_data, month_year, supply_cat, fonts, AccessToken, ipAddr, invoked_date, filepath, subdiv_name)
			report_power_off_duration.callreport_power_off_duration(month_data, month_year, supply_cat, fonts, AccessToken, ipAddr, hierarchy_code, invoked_date, filepath, subdiv_name)
			report_summary.callreport_summary(month_data, month_year, supply_cat, fonts, AccessToken, ipAddr, hierarchy_code, invoked_date, filepath, subdiv_name)
			report_consumption_comparison.callreport_consumption_comparison(month_data, month_year, supply_cat, fonts, AccessToken, ipAddr, invoked_date, hierarchy_code, filepath, subdiv_name)
			report_tamper.callreport_tamper(month_data, month_year, supply_cat, fonts, AccessToken, ipAddr, hierarchy_code, invoked_date, filepath, subdiv_name)
			report_event_exception.callreport_event_exception(month_data, month_year, supply_cat, fonts, AccessToken, ipAddr, hierarchy_code, invoked_date, filepath, subdiv_name)
		}
		else {
