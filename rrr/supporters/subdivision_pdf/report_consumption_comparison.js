...
function callreport_consumption_comparison(month_data, month_year, supply_cat, fonts, AccessToken, ipAddr, invoked_date, hierarchy_code, filepath, subdiv_name) {
					...
					if (merged != undefined) {
						...
						if (final_cons_table.length != 0) {
							...
							table_format.push({
								...
								table: {
									...
									body: [
										...
										[{ text: 'Division Name', style: 'contentHeader' }, dm[0]['div_name'], { text: 'Sub Division Name', style: 'contentHeader' }, dm[0]['subdiv_name']]
									]
								}
							})
							...
							for (var i = 0; i < dm.length; i++) {
								...
								table_rows.push([{ text: i + 1, style: 'tableRows' }, { text: dm[i]['cons_no'], style: 'tableRows' },
							...
							pdfDoc.pipe(fs.createWriteStream(filepath + invoked_date + "/" + subdiv_name + "/" + supply_cat + "_Consumption_Comparison_Report.pdf"));
							pdfDoc.end();
