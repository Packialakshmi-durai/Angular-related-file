...
function callreport_power_off_duration(month_data, month_year, supply_cat, fonts, AccessToken, ipAddr, hierarchy_code, invoked_date, filepath, div_name) {
			...
			try {
				...
				if (cons_data.length != 0 && dm_all.length != 0) {
					...
					if (dm != undefined && dm.length != 0) {
						...
						pdfDoc.pipe(fs.createWriteStream(filepath + invoked_date + "/" + div_name + "/" + supply_cat + "_Power_off_Duration_Report.pdf"));
						pdfDoc.end();
