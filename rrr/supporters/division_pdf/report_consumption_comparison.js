...
function callreport_consumption_comparison(month_data, month_year, supply_cat, fonts, AccessToken, ipAddr, invoked_date, hierarchy_code, filepath, div_name) {
				...
				if (d3_data.length != 0) {
					...
					if (merged != undefined) {
						...
						if (final_cons_table.length != 0) {
							...
							table_format.push({
								...
								table: {
									...
									body: [
										...
										[{ text: 'Division Name', style: 'contentHeader' }, dm[0]['div_name'], {}, {}]
									]
								}
							})
							...
							pdfDoc.pipe(fs.createWriteStream(filepath + invoked_date + "/" + div_name + "/" + supply_cat + "_Consumption_Comparison_Report.pdf"));
							pdfDoc.end();
