...
function callreport_daily(div_result, fonts, AccessToken, ipAddr, hierarchy_code, from, to, invoked_date, supply_cat, filepath, div_name) {
			...
			try {
				...
				if (daily_data.length != 0) {
					...
					if (daily_df.length != 0) {
						...
						template_func(daily_df, fonts, AccessToken, ipAddr, from, to, status_report, invoked_date, supply_cat, filepath, div_name)
					}
					else {
						var errorlog_format = []
						...
						status_report.action = -1
						updateApi(AccessToken, ipAddr, status_report, invoked_date)
					}
				}
				else {
					var errorlog_format = []
...
var template_func = function (daily_df, fonts, AccessToken, ipAddr, from, to, status_report, invoked_date, supply_cat, filepath, div_name) {
	...
	return new Promise(function (resolve, reject) {
		...
		pdfDoc.pipe(fs.createWriteStream(filepath + invoked_date + "/" + div_name + "/" + supply_cat + "_Daily_Report.pdf"));
		pdfDoc.end();
...
var updateApi = function (AccessToken, ipAddr, status_report, invoked_date) {

	client.get(ipAddr + "/api/pdfprocesses?access_token=" + AccessToken + "&filter[where][invoked_date]=" + invoked_date, function (pdf_data, pdf_response, pdf_err) {

		var args_pdfproc = {
			data: {
				action: status_report.action,
				status: "complete",
			},
			headers: {
				"Content-Type": "application/json"
