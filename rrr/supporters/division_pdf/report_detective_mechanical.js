...
function callreport_detective_mechanical(month_data, month_year, supply_cat, fonts, AccessToken, ipAddr, invoked_date, filepath, div_name) {
				...
				if (reading_data.length != 0) {
					...
					if (merged != undefined && merged.length != 0) {
						...
						if (dm != undefined && dm.length != 0) {
							...
							pdfDoc.pipe(fs.createWriteStream(filepath + invoked_date + "/" + div_name + "/" + supply_cat + "_List_of_Detective_Mechanical_Meters_Report.pdf"));
							pdfDoc.end();
