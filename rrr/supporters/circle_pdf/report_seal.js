...
function callreport_seal(month_data, month_year, supply_cat, fonts, AccessToken, ipAddr, invoked_date, crnt_dir) {
				...
				if (reading_data.length != 0) {
					...
					if (merged != undefined && merged.length != 0) {
						...
						if (dm != undefined && dm.length != 0) {
							...
							pdfDoc.pipe(fs.createWriteStream(crnt_dir + "/PDF/" + month_year + "/" + invoked_date + "/" + supply_cat + "_Seal_Report.pdf"));
							pdfDoc.end();
