...
function callreport_daily(fonts, AccessToken, ipAddr, hierarchy_code, from, to, invoked_date, supply_cat, crnt_dir, month_year) {
	...
	return new Promise(function (resolve, reject) {
		...
		client.get(ipAddr + "/api/meters?access_token=" + AccessToken + "&filter[where][hierarchy_code][like]=" + hierarchy_code + "&filter[where][meter_status]=1" + "&filter[where][supply_cat]=" + supply_cat, async function (meter_data, meter_response, meter_err) {
			...
			client.get(ipAddr + "/api/dailies?access_token=" + AccessToken + "&filter[where][hierarchy_code][like]=" + hierarchy_code + "&filter[where][and][0][date_obj][gte]=" + from_date_iso + "&filter[where][and][1][date_obj][lte]=" + to_date_iso + "&filter[order]=date_obj ASC", function (daily_data, daily_response, daily_err) {
				...
				try {
					...
					if (daily_data.length != 0 && meter_data.length != 0) {
						...
						for (var i = 0; i < merged.length; i++) {
							if (merged[i]['meter_no'] != undefined && merged[i]['reports'] != undefined) {
						...
						if (daily_df.length != 0) {
							...
							template_func(daily_df, fonts, AccessToken, ipAddr, from, to, invoked_date, supply_cat, crnt_dir, month_year)
						}
						else {
							var errorlog_format = []
...
var template_func = function (daily_df, fonts, AccessToken, ipAddr, from, to, invoked_date, supply_cat, crnt_dir, month_year) {
	...
	return new Promise(function (resolve, reject) {
		...
		for (var i = 0; i < json.length; i++) {
			...
			if (json[i].bill_mf != null)
				bill_mf = parseFloat(json[i].bill_mf.toString().match(/^-?\d+(?:\.\d{0,2})?/)[0]).toFixed(2)
			if (json[i].contract_demand != null)
				contract_demand = parseFloat(json[i].contract_demand.toString().match(/^-?\d+(?:\.\d{0,2})?/)[0]).toFixed(2)
			...
			table_content = [{
				...
				table: {
					widths: ['*', '*', '*', '*'],
					body: [
						[{ text: 'Division Name', style: 'subheader' }, { text: json[i].divname, style: 'content' },
						{ text: 'Sub Division Name', style: 'subheader' }, { text: json[i].subname, style: 'content' }],

						[{ text: 'Consumer No', style: 'subheader' }, { text: json[i].consumer_no, style: 'content' },
						{ text: 'Consumer Name', style: 'subheader' }, { text: (json[i].consumer_name).substring(0, 20), style: 'content' }],

						[{ text: 'Meter No', style: 'subheader' }, { text: json[i].meter_no, style: 'content' },
		...
		if (!fs.existsSync(invoked_date)) {
			...
			process.chdir(crnt_dir + "/Daily/" + month_year + "/" + invoked_date + "/")
		}
		else {
			process.chdir(crnt_dir + "/Daily/" + month_year + "/" + invoked_date + "/")
		}
		...
		pdfDoc.pipe(fs.createWriteStream(crnt_dir + "/Daily/" + month_year + "/" + invoked_date + "/" + supply_cat + "_Daily_Report.pdf"));
		pdfDoc.end();
