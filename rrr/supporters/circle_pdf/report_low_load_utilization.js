...
function callreport_low_load_utilization(month_data, month_year, supply_cat, fonts, AccessToken, ipAddr, invoked_date, crnt_dir) {
	...
	return new Promise(function (resolve) {
		...
		try {
			...
			if (dm.length != 0) {
				...
				pdfDoc.pipe(fs.createWriteStream(crnt_dir + "/PDF/" + month_year + "/" + invoked_date + "/" + supply_cat + "_Low_Load_Utilization_Report.pdf"));
				pdfDoc.end();

				global.status.cnt = global.status.cnt - 1
				console.log("Low Load Utilization Over : " + global.status.cnt)
